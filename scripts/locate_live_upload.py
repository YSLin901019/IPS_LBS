...
def post_position(
    utm_url: str, utm_key: str, map_id: int,
    row: int, col: int, confidence: float,
    x: float, y: float, z: float = 0.0,
) -> None:
    url = f"{utm_url.rstrip('/')}/api/indoor/position"
    payload = _json.dumps({
        "map_id": map_id, "row": row, "col": col,
        "confidence": confidence, "x": x, "y": y, "z": z,
    }).encode()
    ...
    try:
        with urllib.request.urlopen(req, timeout=5) as resp:
            print(f"  → UTM updated: row={row} col={col}  x={x:.2f}m y={y:.2f}m z={z:.2f}m (HTTP {resp.status})")
